...
from schemas import (
    ...
    TodoItemResponse, TodoListResponse, SuccessResponse,
...
from user_service import authenticate_user, get_user_by_email, create_user
...
def iso_utc(dt: datetime) -> str:
    """Convert datetime to ISO 8601 string in UTC with timezone info"""
...
async def signup(user_create: UserCreate, session=Depends(get_async_session)):
    """Register a new user"""
    # Debug: check what FastAPI receives
    logger.info(f"Signup request received: {user_create.dict()}")
    ...
    # Create new user with safe password hashing
    db_user = await create_user(session, user_create)
...
@app.post("/api/auth/login", response_model=AuthResponse)
async def login(user_login: UserLogin, session=Depends(get_async_session)):
    """Authenticate user and return JWT token"""
    # Debug: check login payload
    logger.info(f"Login request received: {user_login.dict()}")
...
async def get_current_user_details(current_user: dict = Depends(get_current_user), session=Depends(get_async_session)):
    ...
    user = await get_user_by_email(session, current_user["user_id"])
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="User not found"
...
@app.get("/api/todos", response_model=TodoListResponse)
async def get_todos(current_user: dict = Depends(get_current_user), session=Depends(get_async_session)):
    user_id = current_user["user_id"]
    todos = await get_todos_for_user(session, user_id)
...
# @app.post("/api/todos", response_model=TodoResponse, status_code=status.HTTP_201_CREATED)
# async def create_new_todo(todo_request: TodoCreateRequest, current_user: dict = Depends(get_current_user), session=Depends(get_async_session)):
#     user_id = current_user["user_id"]
...
async def not_found_handler(request, exc):
    return JSONResponse(
        status_code=404,
        content=ErrorResponse(success=False, error={"code": "NOT_FOUND", "message": "Resource not found"}).dict()
    )
...
async def unauthorized_handler(request, exc):
    return JSONResponse(
        status_code=401,
        content=ErrorResponse(success=False, error={"code": "UNAUTHORIZED", "message": "Not authenticated"}).dict()
    )
...
async def internal_error_handler(request, exc):
    ...
    return JSONResponse(
        status_code=500,
        content=ErrorResponse(success=False, error={"code": "INTERNAL_ERROR", "message": "Internal server error occurred"}).dict()
    )